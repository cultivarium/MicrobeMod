...
def load_fasta(fasta, bam_file, methylation_type, chunk_size = 10000):
	'''
    ...
    for ref in refs:
        while True:
            if i+chunk_size <= len(refs[ref]):
                ...
                segments.append((ref,bam_file,i,refs[ref][1],methylation_type))
                break 
...
def create_mod_table(args):
    ...
    (ref, bam_file, start, stop, methylation_type) = args
    print(str(start) + ",", end='')
    
    with ModBam(bam_file) as bam:
        positions, counts = bam.pileup(
            ref, start, stop,
            low_threshold=0.33, high_threshold=0.66, mod_base=METHYLATION_TYPES[methylation_type])        
...
def main(bam_file, fasta_file, methylation_types, threads):
    ...
    for methylation_type in methylation_types.split(","):
    	refs, segments = load_fasta(ref_file, bam, methylation_type)
    	results = pool.map(create_mod_table,segments)    
...
if __name__ == '__main__':
	...
	parser.add_argument("-m", "--methylation_types", action="store", default="6mA,5mC", required=True, \
